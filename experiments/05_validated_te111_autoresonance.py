"""Run a separate 10,000-step TE111 autoresonance validation experiment."""
...
def main() -> None:
    ...
    run_id = f"validated_te111_{timestamp}"
    run_directory = Path("runs/validated_te111") / timestamp
    ...
    experiment = Experiment.create(
        config, domain, modules=[resonator, magnetic_field]
    )
    ...
    collector.save_summary(
        ...
        metadata={
            "run_id": run_id,
            "scenario": "validated_te111_autoresonance",
            "steps": args.steps,
            "dt_s": config.dt_s,
            ...
            "magnetic_ramp_time_s": config.magnetic_field.ramp_time_s,
            "nonphysical_scaled_ramp": config.magnetic_field.nonphysical_scaled_ramp,
        },
    )
